...
static uint8_t srec_charToHex(uint8_t ch)
{
    uint8_t retVal = 0xFF;
    ...
    if (srecLine[0] == 'S')
    {
        ...
        if ((errorCode == parseError_Inprogress) || (errorCode == parseError_Done))
        {
            ...
            for (dataIndex  = 0; dataIndex < numberOfAddressByte; dataIndex++)
            {
                ...
                offset +=2;
                calchecksum += dataTpm;  //lay so byte cua address
            }
            // update address
            ...
            for (dataIndex  = 0; dataIndex < output->length; dataIndex++)
            {
                ...
                offset +=2;
                calchecksum += dataTpm; // lay so byte cua data
            }
            ...
            calchecksum = ~calchecksum & 0xFF;
            actualCheckSum = (srec_charToHex(srecLine[offset]) <<4) + srec_charToHex(srecLine[offset +1]);
